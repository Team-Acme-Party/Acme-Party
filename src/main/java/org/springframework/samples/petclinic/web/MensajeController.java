...
public class MensajeController {
		...
		public String initCreationFormMensaje(final ModelMap model) {
			try {
				...
				if (username == null) {
					...
					List<Buzon>buzones = new ArrayList<Buzon>();
					for(User s: usuarios) {
						Patrocinador patrocinador = this.patrocinadorService.findByUsername(s.getUsername());
						Cliente cliente = this.clienteService.findByUsername(s.getUsername());
						Propietario propietario = this.propietarioService.findByUsername(s.getUsername());
						Administrador admin = this.administradorService.findByUsername(s.getUsername());
						if (patrocinador != null) {
							buzones.add(patrocinador.getBuzon());

						} else if (cliente != null) {
							buzones.add(cliente.getBuzon());

						} else if (propietario != null) {
							buzones.add(propietario.getBuzon());;

						} else  {
							buzones.add(admin.getBuzon());
						}
					}
					model.put("buzones",buzones);
	...
	public String processCreationFormMensaje(@Valid Mensaje mensaje, final BindingResult result, final ModelMap model) {
		if (result.hasErrors()) {
			...
			if (patrocinador != null) {
				mensaje.setBuzonRemitente(patrocinador.getBuzon());

			} else if (cliente != null) {
				mensaje.setBuzonRemitente(cliente.getBuzon());

			} else if (propietario != null) {
				mensaje.setBuzonRemitente(propietario.getBuzon());

			} else if (admin != null) {
				mensaje.setBuzonRemitente(admin.getBuzon());
