...
public interface SpringDataComentarioRepository extends ComentarioRepository, Repository<Comentario, Integer> {
	...
	@Override
	@Query("SELECT comentario FROM Comentario comentario WHERE comentario.fiesta =:id")
	Collection<Comentario> findByFiestaId(@Param("id") int id);

	@Override
	@Query("SELECT comentario FROM Comentario comentario WHERE comentario.local =:id")
