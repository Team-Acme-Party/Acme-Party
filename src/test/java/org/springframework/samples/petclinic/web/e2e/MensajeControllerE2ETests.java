...
public class MensajeControllerE2ETests {
	...
	void testDetallesMensaje() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("/mensajes/{mensajeId}", 4)).andExpect(MockMvcResultMatchers.status().is2xxSuccessful()).andExpect(MockMvcResultMatchers.view().name("mensajes/mensajeDetails"));
	}
	...
	public void testInitCreationForm() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("/mensajes/new")).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.model().attributeExists("mensaje")).andExpect(MockMvcResultMatchers.view().name("mensajes/new"));
	}
	...
	void testCreateMensaje() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.post("/mensajes/new").with(SecurityMockMvcRequestPostProcessors.csrf()).param("asunto", "asunto prueba").param("cuerpo", "cuerpo prueba").param("destinatario", "administrador"))
			.andExpect(MockMvcResultMatchers.status().is3xxRedirection()).andExpect(MockMvcResultMatchers.view().name("redirect:/usuario/mensajes"));
	}
	...
	public void testProcessCreationFormHasErrors() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.post("/mensajes/new").with(SecurityMockMvcRequestPostProcessors.csrf()).param("cuerpo", "cuerpo prueba").param("destinatario", "administrador")).andExpect(MockMvcResultMatchers.status().is2xxSuccessful())
